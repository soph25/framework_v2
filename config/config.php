...
return [
    ...
    'admin.prefix' => '/admin',  
    'blog.path' => 'C:\xampp\htdocs\onisep\src\Blog\views', 
    ...
    'plates'   => function (\Psr\Container\ContainerInterface $container) {
     $path_info = !empty($_SERVER['PATH_INFO']) ? $_SERVER['PATH_INFO'] : (!empty($_SERVER['ORIG_PATH_INFO']) ? $_SERVER['ORIG_PATH_INFO'] : '');
        ...
        //var_dump($dir);
        //die();   
        $view =  new \League\Plates\Engine('C:\xampp\htdocs\onisep\src\Blog\views');
        $view->loadExtension(new \League\Plates\Extension\URI($path_info)); 
		$view->loadExtension(new \Framework\Plate\Csrf($container->get(Framework\Middleware\CsrfMiddleware::class))); 
    ...
    'home'  => \DI\object(\League\Plates\Engine::class)->constructor('C:\xampp\htdocs\onisep\src\Blog\views'),
    'lobin'   => function (\Psr\Container\ContainerInterface $container) {
      $path_info = !empty($_SERVER['PATH_INFO']) ? $_SERVER['PATH_INFO'] : (!empty($_SERVER['ORIG_PATH_INFO']) ? $_SERVER['ORIG_PATH_INFO'] : '');  
      $vue =  new \League\Plates\Engine('C:\xampp\htdocs\onisep\src\Blog\views'); 
    ...
    \PDO::class => function (\Psr\Container\ContainerInterface $c) {
		...
		$DB = new \PDO('mysql:host=localhost;dbname=07795_onisup', 'root', 'froggy25');
